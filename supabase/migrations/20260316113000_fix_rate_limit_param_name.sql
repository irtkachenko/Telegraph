-- Avoid ambiguous column references without renaming parameters (keeps dependencies intact)
...
begin
  ...
  if check_action_limit.u_id is null then
    raise exception 'Unauthenticated' using errcode = '28000';
  end if;

  if check_action_limit.max_count is null or check_action_limit.seconds is null then
    ...
    cfg_max := check_action_limit.max_count;
    cfg_seconds := check_action_limit.seconds;
    cfg_enabled := true;
  end if;
  ...
  values (check_action_limit.u_id, check_action_limit.action, cfg_seconds, window_start_ts, 1)
