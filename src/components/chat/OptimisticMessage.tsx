...
const OptimisticAttachmentComponent = memo(({ attachment }: { attachment: OptimisticAttachment }) => {
...
export function OptimisticMessage({ message }: OptimisticMessageProps) {
  ...
  const hasUploadingAttachments = message.attachments?.some((att: any) => att.uploading);
  const hasFailedAttachments = message.attachments?.some((att: any) => att.error);
    ...
    <div className={cn(
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="text-[10px] text-gray-500 mt-1 text-center">
          {isOptimistic ? (
            hasUploadingAttachments ? "Uploading" : 
            hasFailedAttachments ? "Failed" : "Sending"
          ) : (
            "Sent"
          )}
        </div>
      ...
      <div className="flex-1 min-w-0">
        ...
        {hasUploadingAttachments && (
          <div className="flex items-center gap-2 text-xs text-blue-400 bg-blue-500/10 p-2 rounded">
            ...
            <span>
              Uploading {message.attachments?.filter((att: any) => att.uploading).length} file(s)...
            </span>
          </div>
        )}
      </div>
